...
Tile *Grid::GetTarget() { return this->target; }
...
void Tile::render(SDL_Renderer *renderer, int x, int y) {
  ...
  if (this->hasDeflector) {
    if (this->direction == DeflectorDirection::Back) {
      SDL_SetRenderDrawColor(renderer, 0, 0, 255, 255);
      SDL_RenderDrawLine(renderer, x * GRID_SPACING, y * GRID_SPACING,
                         (x * GRID_SPACING) + GRID_SPACING,
                         (y * GRID_SPACING) + GRID_SPACING);
      SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255);
    } else {
      SDL_SetRenderDrawColor(renderer, 0, 0, 255, 255);
      SDL_RenderDrawLine(renderer, x * GRID_SPACING + GRID_SPACING,
                         y * GRID_SPACING, x * GRID_SPACING,
                         y * GRID_SPACING + GRID_SPACING);
      SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255);
    }
  }
...
float Tile::CenterX() { return this->x + (1 / 2); }

float Tile::CenterY() { return this->y + (1 / 2); }

int Tile::Left() { return this->x; }

int Tile::Right() { return this->x + 1; }

int Tile::Top() { return this->y; }

int Tile::Bottom() { return this->y + 1; }
